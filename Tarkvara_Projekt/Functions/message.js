function message () {
    ...
    $message.on('click', function messageWindow () {
        game();
        ...
        $arrow.on("click", () => {
            ...
            //$.getScript("Functions/main.js", () => createMainMenu());
            createMainMenu();
        });
...
function createSender (senderName) {
    const $content = $('#content');
    const $block = $('<div></div>').attr('id', senderName).css({"width": "90%",
    "margin": "0 auto",
    "border-bottom": "1px solid grey",
    "cursor": "pointer"});
    $content.append($block);
    const $sender = $('<h3></h3>').attr('class', 'sender').text(senderName).css({"margin": "10px 0 5px 0"});
    const $unreadedMesseges = $('<p></p>').attr('class', 'unreadedMesseges').css ({
        "margin": "-10px 0px 5px",
        "float": "right",
        "background": "red",
        "padding": "5px 10px",
    ...
    const $senderText = $('<p></p>').attr('class', 'senderText').text('...').css({"margin": "0 0 5px 0",
                      "color": "grey",
                      "text-align": "left"});
    $block.append($sender);
