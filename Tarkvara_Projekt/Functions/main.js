...
function snakeWindow () {
    const $content = $('#content');
    const $appBlock = $('#appBlock');
    $content.css({"background-image": "unset", "background-color": "grey"});
    $appBlock.remove();
    $content.empty();
    const $arrow = $("<i></i>").attr("class", "fa fa-arrow-left").css({
        "padding-top": "15px",
    ...
    $('.fa-arrow-left').on("click", () => {
        ...
        $(function() {
            $message.animate({
                ...
                complete: function() {
                    $message.remove();
                    if (window.snakeTimer) clearInterval(snakeTimer);
                    createMainMenu(true);
                }
            })
        })
    });
    $snakeHeader = $('<h1></h1>').attr('class', 'snake-header').text('Snake');
    $content.append($snakeHeader);
    $snakeButton = $('<button></button>').attr('class', 'snake-button').text('Start game');
    $snakeButton.on('click', () => {
        ...
        $currentScore = $('<p></p>').attr('class', 'current-score')
        $content.append($currentScore);
        $bestScore = $('<p></p>').attr('class', 'best-score')
...
function createApp (appName) {
    ...
    else if (appName === 'Snake') appIcon = $('<img>').attr('src', 'snake.jpg')
